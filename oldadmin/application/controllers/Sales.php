...
class Sales extends CI_Controller {
    ...
    public function all_orders(){
        $sql = 'SELECT so.*,p.product_title as title from sales_order as so inner join products as p on p.id=so.product_id';
        $query = $this->db->query( $sql);
        $result=  $query->result_array();
        $data = array( 'success'=>true, 'data'=>$result);
    ...
    public function jobberlist(){
        ...
        if($this->user_role=="Admin"){
            ...
            $sql = "SELECT f.*,u.id as user_id FROM franchies as f inner join users as u on u.controller_id=f.Id where f.AssociatesType='Sales Jobber' and u.parent_id='$user_id'";
        }
        $query = $this->db->query($sql);
        $result=$query->result_array();
        $new_result=array();
