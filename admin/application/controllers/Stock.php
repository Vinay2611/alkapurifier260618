...
class Stock extends CI_Controller {
    ...
    public function index()
    {
        ...
        if($this->user_role=="Admin"){
            $sql = 'SELECT s.*,p.product_title as title,p.stock_price,p.sales_price,f.FranchiesName FROM stock as s left join products as p on p.id=s.product_id left join users as u on u.id=s.user_id left join franchies as f on f.Id=u.controller_id';
        }elseif($this->user_role=="Area Production Controller"){
            $user_ids=$this->getChildAreaProductionUser($user_id);
            $sql = "SELECT s.*,p.product_title as title,p.stock_price,p.sales_price,f.FranchiesName FROM stock as s left join products as p on p.id=s.product_id left join users as u on u.id=s.user_id left join franchies as f on f.Id=u.controller_id where s.user_id IN ($user_ids)";
        }elseif($this->user_role=="Plant Production Controller"){
            $user_ids=$this->getChildPlantProductionUser($user_id);
            $sql = "SELECT s.*,p.product_title as title,p.stock_price,p.sales_price,f.FranchiesName FROM stock as s left join products as p on p.id=s.product_id left join users as u on u.id=s.user_id left join franchies as f on f.Id=u.controller_id where s.user_id IN ($user_ids)";
        }elseif($this->user_role=="Industrial Production Controller"){
            $user_ids=$this->getChildIndustrialProductionUser($user_id);
            $sql = "SELECT s.*,p.product_title as title,p.stock_price,p.sales_price,f.FranchiesName FROM stock as s left join products as p on p.id=s.product_id left join users as u on u.id=s.user_id left join franchies as f on f.Id=u.controller_id where s.user_id IN ($user_ids)";
        }elseif($this->user_role=="Production Jobber"){
            $sql = "SELECT s.*,p.product_title as title,p.stock_price,p.sales_price,f.FranchiesName FROM stock as s left join products as p on p.id=s.product_id left join users as u on u.id=s.user_id left join franchies as f on f.Id=u.controller_id where user_id = '$user_id'";
        }
    ...
    public function getChildAreaProductionUser($user_id){
    ...
    public function getChildPlantProductionUser($user_id){
    ...
    public function getChildIndustrialProductionUser($user_id){
